...
#define CLANGCMD "call \"" + Setting::getHome() + "vsvars32.bat\" & " + Setting::getHome() + "clang++ "
	...
	static void Setting::setInfo(int argc,char* argv[])
	{
		...
		case 1: 
			if (inFiles[0].type == M_FILE){
				...
				if (genExe)
					exit(system((CLANGCMD + MSVLHOME + "ThreadLib.lib " + MSVLHOME + "Modeling.lib " + inFiles[0].path + " -o " + outPath + outName).data()));
				else
					exit(system(("COPY " + inFiles[0].path + " " + outPath + outName).data()));
			}
			break;
		default:
			string cmd = CLANGCMD + MSVLHOME + "ThreadLib.lib " + MSVLHOME + "Modeling.lib -o " + outPath + outName + " ";
